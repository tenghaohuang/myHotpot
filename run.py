...
def train(config):
    ...
    for epoch in range(10000):
        for data in build_train_iterator():
            ...
            pred = model(context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, question_lens,start_mapping, end_mapping, all_mapping)
            # with open('output.txt','w') as f:
            #     f.write(str(pred))
            pickle.dump(pred, open("combine.p", "wb"))
            print('lol here is the fking output',pred)
            assert(1==0)
            loss = (nll_sum())
            optimizer.zero_grad()
            loss.backward()
